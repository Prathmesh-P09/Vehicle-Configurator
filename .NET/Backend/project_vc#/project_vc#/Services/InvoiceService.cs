...
public class InvoiceService : IInvoiceService
{
    ...
    public async Task GenerateInvoiceAsync(InvoiceRequestDTO dto, string username)
    {
        ...
        // Needs to reload details with relationships for PDF?
        // Actually we have the data here.
        // But PdfGenerator might expect `d.Comp` to be populated.
        foreach(var d in savedDetails) {
            if(d.Comp == null) d.Comp = selectedComponents.First(c => c.CompId == d.CompId);
        }
        // Actually simplified: We assigned CompId. Relation properties might be null unless we set them or reload.
        // Let's manually set them for PDF generation use.
